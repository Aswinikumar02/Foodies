...
class DisplayFavComponent extends React.Component {
    ...
    render() {
        ...
        let JsonArray = this.props.json.map(function(item) {
            if (fav === 'favourites') {
                return <CardsComponent id={item._id} name={item.name} img={item.img} address={item.address} cuisines={item.cuisines} ratings={item.ratings} comments={item.comments} fav='favourites' removeFav={removeFav}/>
            }
        });
        return (
            <div style={divStyle}>
